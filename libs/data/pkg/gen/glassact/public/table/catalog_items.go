//
// Code generated by go-jet DO NOT EDIT.
//
// WARNING: Changes to this file may cause incorrect behavior
// and will be lost if the code is regenerated
//

package table

import (
	"github.com/go-jet/jet/v2/postgres"
)

var CatalogItems = newCatalogItemsTable("public", "catalog_items", "")

type catalogItemsTable struct {
	postgres.Table

	// Columns
	ID                  postgres.ColumnInteger
	UUID                postgres.ColumnString
	CatalogCode         postgres.ColumnString
	Name                postgres.ColumnString
	Description         postgres.ColumnString
	Category            postgres.ColumnString
	DefaultWidth        postgres.ColumnFloat
	DefaultHeight       postgres.ColumnFloat
	MinWidth            postgres.ColumnFloat
	MinHeight           postgres.ColumnFloat
	DefaultPriceGroupID postgres.ColumnInteger
	SvgURL              postgres.ColumnString
	Manifest            postgres.ColumnString
	IsQuarantined       postgres.ColumnBool
	QuarantineReason    postgres.ColumnString
	IsActive            postgres.ColumnBool
	CreatedAt           postgres.ColumnTimestampz
	UpdatedAt           postgres.ColumnTimestampz
	Version             postgres.ColumnInteger

	AllColumns     postgres.ColumnList
	MutableColumns postgres.ColumnList
	DefaultColumns postgres.ColumnList
}

type CatalogItemsTable struct {
	catalogItemsTable

	EXCLUDED catalogItemsTable
}

// AS creates new CatalogItemsTable with assigned alias
func (a CatalogItemsTable) AS(alias string) *CatalogItemsTable {
	return newCatalogItemsTable(a.SchemaName(), a.TableName(), alias)
}

// Schema creates new CatalogItemsTable with assigned schema name
func (a CatalogItemsTable) FromSchema(schemaName string) *CatalogItemsTable {
	return newCatalogItemsTable(schemaName, a.TableName(), a.Alias())
}

// WithPrefix creates new CatalogItemsTable with assigned table prefix
func (a CatalogItemsTable) WithPrefix(prefix string) *CatalogItemsTable {
	return newCatalogItemsTable(a.SchemaName(), prefix+a.TableName(), a.TableName())
}

// WithSuffix creates new CatalogItemsTable with assigned table suffix
func (a CatalogItemsTable) WithSuffix(suffix string) *CatalogItemsTable {
	return newCatalogItemsTable(a.SchemaName(), a.TableName()+suffix, a.TableName())
}

func newCatalogItemsTable(schemaName, tableName, alias string) *CatalogItemsTable {
	return &CatalogItemsTable{
		catalogItemsTable: newCatalogItemsTableImpl(schemaName, tableName, alias),
		EXCLUDED:          newCatalogItemsTableImpl("", "excluded", ""),
	}
}

func newCatalogItemsTableImpl(schemaName, tableName, alias string) catalogItemsTable {
	var (
		IDColumn                  = postgres.IntegerColumn("id")
		UUIDColumn                = postgres.StringColumn("uuid")
		CatalogCodeColumn         = postgres.StringColumn("catalog_code")
		NameColumn                = postgres.StringColumn("name")
		DescriptionColumn         = postgres.StringColumn("description")
		CategoryColumn            = postgres.StringColumn("category")
		DefaultWidthColumn        = postgres.FloatColumn("default_width")
		DefaultHeightColumn       = postgres.FloatColumn("default_height")
		MinWidthColumn            = postgres.FloatColumn("min_width")
		MinHeightColumn           = postgres.FloatColumn("min_height")
		DefaultPriceGroupIDColumn = postgres.IntegerColumn("default_price_group_id")
		SvgURLColumn              = postgres.StringColumn("svg_url")
		ManifestColumn            = postgres.StringColumn("manifest")
		IsQuarantinedColumn       = postgres.BoolColumn("is_quarantined")
		QuarantineReasonColumn    = postgres.StringColumn("quarantine_reason")
		IsActiveColumn            = postgres.BoolColumn("is_active")
		CreatedAtColumn           = postgres.TimestampzColumn("created_at")
		UpdatedAtColumn           = postgres.TimestampzColumn("updated_at")
		VersionColumn             = postgres.IntegerColumn("version")
		allColumns                = postgres.ColumnList{IDColumn, UUIDColumn, CatalogCodeColumn, NameColumn, DescriptionColumn, CategoryColumn, DefaultWidthColumn, DefaultHeightColumn, MinWidthColumn, MinHeightColumn, DefaultPriceGroupIDColumn, SvgURLColumn, ManifestColumn, IsQuarantinedColumn, QuarantineReasonColumn, IsActiveColumn, CreatedAtColumn, UpdatedAtColumn, VersionColumn}
		mutableColumns            = postgres.ColumnList{UUIDColumn, CatalogCodeColumn, NameColumn, DescriptionColumn, CategoryColumn, DefaultWidthColumn, DefaultHeightColumn, MinWidthColumn, MinHeightColumn, DefaultPriceGroupIDColumn, SvgURLColumn, ManifestColumn, IsQuarantinedColumn, QuarantineReasonColumn, IsActiveColumn, CreatedAtColumn, UpdatedAtColumn, VersionColumn}
		defaultColumns            = postgres.ColumnList{IDColumn, UUIDColumn, ManifestColumn, IsQuarantinedColumn, IsActiveColumn, CreatedAtColumn, UpdatedAtColumn, VersionColumn}
	)

	return catalogItemsTable{
		Table: postgres.NewTable(schemaName, tableName, alias, allColumns...),

		//Columns
		ID:                  IDColumn,
		UUID:                UUIDColumn,
		CatalogCode:         CatalogCodeColumn,
		Name:                NameColumn,
		Description:         DescriptionColumn,
		Category:            CategoryColumn,
		DefaultWidth:        DefaultWidthColumn,
		DefaultHeight:       DefaultHeightColumn,
		MinWidth:            MinWidthColumn,
		MinHeight:           MinHeightColumn,
		DefaultPriceGroupID: DefaultPriceGroupIDColumn,
		SvgURL:              SvgURLColumn,
		Manifest:            ManifestColumn,
		IsQuarantined:       IsQuarantinedColumn,
		QuarantineReason:    QuarantineReasonColumn,
		IsActive:            IsActiveColumn,
		CreatedAt:           CreatedAtColumn,
		UpdatedAt:           UpdatedAtColumn,
		Version:             VersionColumn,

		AllColumns:     allColumns,
		MutableColumns: mutableColumns,
		DefaultColumns: defaultColumns,
	}
}
