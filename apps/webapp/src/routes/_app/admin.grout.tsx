...
import { Grout, GET } from "@glassact/data";
...
export const Route = createFileRoute("/_app/admin/grout")({
...
function GroutForm(props: GroutFormProps) {
  ...
  return (
    ...
    >
      <form.Field
        name="name"
        children={(field) => (
          <Form.TextField field={field} label="Name" placeholder="Grout name" />
        )}
      />
...
const defaultColumns: ColumnDef<GET<Grout>>[] = [
  {
    id: "actions",
    enableHiding: false,
    header: "Actions",
    cell: (props) => {
      ...
      const deleteMutation = useMutation(() =>
        deleteGroutOpts(props.row.original.uuid),
      );
      const patchMutation = useMutation(() =>
        patchGroutOpts(props.row.original.uuid),
      );

      const invalidate = () =>
        queryClient.invalidateQueries({ queryKey: ["grouts"] });
...
function EditButton(props: EditButtonProps) {
  ...
  const queryClient = useQueryClient();
  const patchMutation = useMutation(() => patchGroutOpts(props.item.uuid));
  ...
  return (
    <Dialog open={isOpen()} onOpenChange={setIsOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Edit Grout</DialogTitle>
        </DialogHeader>
        <GroutForm
          defaultValues={{
...
function RouteComponent() {
  ...
  const query = useQuery(() => getGroutsAdminOpts({ limit: 99, offset: 0 }));
  const postMutation = useMutation(() => postGroutOpts());
  const queryClient = useQueryClient();
    ...
    <div>
      <div class="flex items-center justify-between py-4">
        ...
        <Dialog open={isCreateDialogOpen()} onOpenChange={setIsCreateDialogOpen}>
          <DialogTrigger as={Button}>Add a new grout</DialogTrigger>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Add a new grout</DialogTitle>
            </DialogHeader>
            <GroutForm
